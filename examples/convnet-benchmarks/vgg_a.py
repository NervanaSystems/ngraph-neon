...
# ******************************************************************************
"""
Convnet-VGG_A Benchmark with spelled out neon model framework in one file
...
parser = NeonArgparser(description='Train convnet-vgg_a model on random dataset')
...
# Setup model
seq1 = Sequential([Convolution((3, 3, 64), filter_init=GaussianInit(std=0.01),
                               bias_init=init,
                               activation=Rectlin(), padding=1),
                   Pooling((2, 2), strides=2),
                   Convolution((3, 3, 128), filter_init=GaussianInit(std=0.01),
                               bias_init=init,
                               activation=Rectlin(), padding=1),
                   Pooling((2, 2), strides=2),
                   Convolution((3, 3, 256), filter_init=GaussianInit(std=0.01),
                               bias_init=init,
                               activation=Rectlin(), padding=1),
                   Convolution((3, 3, 256), filter_init=GaussianInit(std=0.01),
                               bias_init=init,
