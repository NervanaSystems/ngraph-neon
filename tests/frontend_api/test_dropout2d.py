...
import ngraph as ng
from ngraph.frontends.neon.layer import Layer, Dropout2D
...
@pytest.mark.parametrize("t, nin,batch_size", [(100, 32, 2)])
@pytest.mark.parametrize("keep", [1.0, 0.75, 0.5])
def test_dropout2d_train(t, nin, batch_size, keep):

    # set inputs
    N = ng.make_axis(batch_size, name='N')
    T = ng.make_axis(t, name='T')
    C = ng.make_axis(nin, name='C')

    inp = ng.placeholder([C, T, N])
    layer = Dropout2D(keep=keep)
    fprop = layer(inp)

    # create data
    x = np.random.uniform(size=(nin, t, batch_size))

    # evaluate
    with ExecutorFactory() as ex:
        comp = ex.executor([fprop, layer.mask], inp)
        out, mask = comp(x)
        numpy_out = x * mask[:, np.newaxis, np.newaxis]
...
@pytest.mark.parametrize("t, nin,batch_size", [(100, 32, 2)])
def test_dropout2d_inference(t, nin, batch_size):
    # set inputs
    N = ng.make_axis(batch_size, name='N')
    T = ng.make_axis(t, name='T')
    C = ng.make_axis(nin, name='C')
...
def test_dropout2d_bprop_single_comp(t, nin, batch_size, keep):
    # set inputs
    N = ng.make_axis(batch_size, name='N')
    T = ng.make_axis(t, name='T')
    C = ng.make_axis(nin, name='C')
    ...
    with ExecutorFactory() as ex:
        ...
        ng.testing.assert_allclose(bout, (x * mask[:, np.newaxis, np.newaxis]).sum(), rtol=1e-6)
