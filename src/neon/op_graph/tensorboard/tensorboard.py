...
import numpy as np
from neon.op_graph.tensorboard import summary
from neon.op_graph.tensorboard.graph_def import neon_to_tf_graph_def
from neon.op_graph.tensorboard.record_writer import RecordWriter, create_event
...
class TensorBoard(object):
    ...
    def __init__(self, logdir, run=None):
        """
        Creates an interface for logging neon data to tensorboard
    ...
    def add_run(self, run=None):
        ...
        def get_events_filename():
            return ".".join(["events.neon.out.tfevents",
                             str(time.time())[:10],
                             socket.gethostname()])
    ...
    def add_graph(self, ops):
        ...
        """
        graph_def = neon_to_tf_graph_def(ops)
