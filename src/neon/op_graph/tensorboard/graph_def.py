...
from neon.op_graph.serde.serde import _serialize_graph
from neon.op_graph.tensorboard.summary import _clean_tag


def neon_to_tf_graph_def(graph):
    # pylint: disable=line-too-long
    """
    Given an neon graph, convert it to a TensorFlow `GraphDef` protobuf object in memory.
    ...
    for op in ng_graph_def.ops:
        ...
        for key, value in op.attrs.items():
            if key.startswith("_neon_metadata_"):
                key = key.replace("_neon_metadata_", "")
