...
# ******************************************************************************
"""
This module handles neon IR topology serialization. The general approach for serialization is to

1. Take in a list of neon Ops, and use the new Op.all_op_references method to grab (recursively)
...
import neon
import neon.op_graph.op_graph as op_graph
from neon.op_graph.op_graph import Op
from neon.op_graph.axes import Axes, Axis, FlattenedAxis, AxesMap

import neon.op_graph.serde.ops_pb2 as ops_pb
...
def assign_scalar(message, value):
    ...
    if value is None:
        ...
        assign_scalar(message.map_val.map['_neon_map_sentinel_'], '')
...
def assign_op_attr(message, value):
    ...
    if is_scalar_type(value):
        ...
        if len(value) > 0:
            ...
            assign_scalar(message.repeated_scalar.val.add(), '_neon_iter_sentinel_')
    elif value is None:
...
def op_to_protobuf(op):
    ...
    for key in op.metadata:
        ...
        if key in ('hetr_replaced_by', 'replaces_op', 'layout', 'clones'):
            continue
        assign_op_attr(pb_op.attrs['_neon_metadata_' + key], op.metadata[key])

    if hasattr(op, '_neon_ser_handle'):
        pb_op.attrs['_neon_ser_handle'].scalar.bool_val = True
        # cleanup our sentinel value
        del op._neon_ser_handle
...
def add_edges(pb_edges, pb_ops, op):
    ...
    if op._forward is not None:
        ...
        forward_edge.attrs['_neon_forward'].scalar.bool_val = True
    if hasattr(op, '_args'):
        for arg in op._args:
    ...
    for key in remaining_keys:
        ...
        if isinstance(val, Op):
            ...
            edge.attrs['_neon_attribute'].scalar.string_val = key
        if isinstance(val, (list, tuple, set, OrderedSet)):
            for item in val:
                if isinstance(item, Op):
                    edge = add_edge(op, item, ops_pb.Edge.OTHER)
                    edge.attrs['_neon_list_attribute'].scalar.string_val = key
...
def serialize_graph(ops, only_return_handle_ops=False):
    """
    Dumps neon graph to serialized protobuf byte string
...
def protobuf_to_axes(msg):
    ...
    axes = neon.make_axes(list(map(pb_to_axis, msg.axes)))
...
def protobuf_attr_to_python(val):
    ...
    if val.HasField('tensor'):
        ...
        if len(val.repeated_scalar.val) == 1 and \
                val.repeated_scalar.val[0].string_val == '_neon_iter_sentinel_':
            return ()
        else:
...
def get_neon_op_cls(op_type):
    """ Walk over python modules in neon.op_graph and look for op_type class. """
    for importer, modname, ispkg in pkgutil.iter_modules(neon.op_graph.__path__):
        imported_mod = importlib.import_module('neon.op_graph.' + modname)
        if hasattr(imported_mod, op_type):
            return getattr(imported_mod, op_type)
    raise ValueError("Cannot find op_type of {} in any neon.op_graph modules.".format(op_type))
...
def protobuf_to_op(pb_op):
    ...
    """
    cls = get_neon_op_cls(pb_op.op_type)
    ...
    for key in remaining_keys:
        if key == '_neon_ser_handle':
            py_op._neon_ser_handle = True
        if key.startswith('_neon_metadata_'):
...
def _deserialize_graph(graph_pb):
    ...
    for edge in graph_pb.edges:
        ...
        if edge.edge_type == ops_pb.Edge.DATA:  # args
            ...
            if '_neon_forward' in edge.attrs:  # forward
            ...
            if '_neon_attribute' in edge.attrs:
                setattr(head_op, edge.attrs['_neon_attribute'].scalar.string_val, tail_op)
            elif '_neon_list_attribute' in edge.attrs:
                key = edge.attrs['_neon_list_attribute'].scalar.string_val
